...
class Matrice:
    ...
    def __eq__(self, M: Union[list, Matrice]) -> bool:
    ...
    def __ne__(self, M: Union[list, Matrice]) -> bool:
        if len(self.value) == len(M) and len(self.value[0]) == len(M[0]):
            for i in range(len(M)):
                for j in range(len(M[i])):
                    if self.value[i][j] != M[i][j]:
...
class Vecteur:
    ...
    def __mul__(self, M: Union[Vecteur, Point, float, int, complex]) -> Union[Matrice, Vecteur, int, float]:
        # on note V le vecteur, M la matrice, P le point et k un réel
        # on peut effectuer un produit :
        # V * P => M
        # V * k => V
        # V * V => k produit scalaire
        # M * V => M <- a implémenter dans la classe Matrice
        # M * P => P NON
        # P * M => P <- a implémenter dans la classe Point
        if isinstance(M, Point):  # V * P => M
            m = []
            if len(M) != 1:
                return m
            for i in range(len(self.value)):
    ...
    def plot(self, x: float = 0, y: float = 0, c: str = "") -> None:
        """
        Ajoute au graphique en cours, le vecteur, sous forme d'une fleche à partir de l'origine, ou à partir de x,y données en argument, coleur facultative
        """
...
class Point:
    ...
    def __mul__(self, M: Union[Matrice, float, int]) -> Point:
        # on note V le vecteur, M la matrice, P le point et k un réel
        # on peut effectuer un produit :
        # V * P => M
        # V * k => V
